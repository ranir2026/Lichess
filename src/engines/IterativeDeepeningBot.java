...
public class IterativeDeepeningBot {
    ...
    public IterativeDeepeningBot(Board game, boolean color) {
        this.game = game;
        this.color = color;
    }
    ...
    public int getBestMove(int TotalTimeLeft, int increment) {
        ...
        if ((this.game.getStateTracker().getPly() / 2) <= 4) timeLimit /= 5;
        ...
        for (int currentDepth = 1; currentDepth <= maxDepth; currentDepth++) {
            ...
            // 1. Reset the window for every new depth to find the true best score
            double alpha = Double.NEGATIVE_INFINITY;
            double beta = Double.POSITIVE_INFINITY;
            ...
            // 2. Order moves (Your OrderMoves uses the TT, so it will automatically 
            // put the best move from 'currentDepth - 1' at the front!)
            int[] orderedMoves = OrderMoves(moves);
            ...
            // 3. Update the global best move only after a full depth is completed
